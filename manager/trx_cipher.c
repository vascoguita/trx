...
TEE_Result trx_cipher_encrypt_data(trx_dek *dek, void *data, size_t data_size, unsigned long int version,
                                     void *id, size_t id_size, void *udid, size_t udid_size, void *dst, size_t *dst_size)
{
    TEE_Result res;
    ...
    TEE_AEUpdateAAD(op_handle, udid, udid_size);
...
TEE_Result trx_cipher_encrypt(trx_vk *vk, TEE_UUID *uuid, void *src, size_t src_size,
                              ...
                              void *udid, size_t udid_size, void *dst, size_t *dst_size)
{
    TEE_Result res;
    trx_tsk *tsk = NULL;
    trx_dek *dek = NULL;
    ...
    res = trx_cipher_encrypt_data(NULL, src, src_size, version, id, id_size, udid, udid_size, NULL, &data_enc_size);
    ...
    res = trx_cipher_encrypt_data(dek, src, src_size, version, id, id_size, udid, udid_size, data_enc, &data_enc_size);
    if (res != TEE_SUCCESS)
    {
        goto out;
    }
                              ...
                              void *udid, size_t udid_size, void *dst, size_t *dst_size)
{
    ...
    res = trx_cipher_encrypt_data(NULL, NULL, 0, 0, NULL, 0, NULL, 0, NULL, &data_enc_size);
    if (res != TEE_ERROR_SHORT_BUFFER)
    {
        res = TEE_ERROR_GENERIC;
        goto out;
    ...
    res = trx_cipher_decrypt_data(NULL, data_enc, data_enc_size, version, id, id_size, udid, udid_size, NULL, &exp_dst_size);
    if (res != TEE_ERROR_SHORT_BUFFER)
    {
        res = TEE_ERROR_GENERIC;
        goto out;
    ...
    res = trx_cipher_decrypt_data(dek, data_enc, data_enc_size, version, id, id_size, udid, udid_size, dst, &exp_dst_size);
    if ((res != TEE_SUCCESS) || (exp_dst_size != *dst_size))
    {
        res = TEE_ERROR_GENERIC;
    }
