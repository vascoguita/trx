...
TEE_Result share(void *sess_ctx, uint32_t param_types, TEE_Param params[4])
{
    ...
    if(!(file = trx_file_init(ree_path, ree_path_size))){
        EMSG("TA_TRX_MANAGER_CMD_SHARE failed calling function \'trx_file_init\'");
        Cipher_clear(bk_enc);
        free(ree_path);
        return TEE_ERROR_GENERIC;
    }
    ...
    if((file->bk_enc_size = Cipher_snprint(NULL, 0, bk_enc) + 1) < 1) {
    ...
    if(file->bk_enc_size != (size_t)(Cipher_snprint(file->bk_enc, file->bk_enc_size, bk_enc) + 1)) {
        EMSG("TA_TRX_MANAGER_CMD_SHARE failed calling function \'Cipher_snprint\'");
        Cipher_clear(bk_enc);
        trx_file_clear(file);
        return TEE_ERROR_GENERIC;
